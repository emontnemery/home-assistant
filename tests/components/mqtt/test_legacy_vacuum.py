...
async def test_default_supported_features(hass, mqtt_mock_entry):
...
async def test_all_commands(hass, mqtt_mock_entry):
...
async def test_commands_without_supported_features(hass, mqtt_mock_entry):
...
async def test_attributes_without_supported_features(hass, mqtt_mock_entry):
...
async def test_status(hass, mqtt_mock_entry):
...
async def test_status_battery(hass, mqtt_mock_entry):
    """Test status updates from the vacuum."""
    config = deepcopy(DEFAULT_CONFIG)
    config[mqttvacuum.CONF_SUPPORTED_FEATURES] = services_to_strings(
        ALL_SERVICES, SERVICE_TO_STRING
...
async def test_status_cleaning(hass, mqtt_mock_entry):
    """Test status updates from the vacuum."""
    config = deepcopy(DEFAULT_CONFIG)
    config[mqttvacuum.CONF_SUPPORTED_FEATURES] = services_to_strings(
        ALL_SERVICES, SERVICE_TO_STRING
...
async def test_status_docked(hass, mqtt_mock_entry):
    """Test status updates from the vacuum."""
    config = deepcopy(DEFAULT_CONFIG)
    config[mqttvacuum.CONF_SUPPORTED_FEATURES] = services_to_strings(
        ALL_SERVICES, SERVICE_TO_STRING
...
async def test_status_fan_speed(hass, mqtt_mock_entry):
    """Test status updates from the vacuum."""
    config = deepcopy(DEFAULT_CONFIG)
    config[mqttvacuum.CONF_SUPPORTED_FEATURES] = services_to_strings(
        ALL_SERVICES, SERVICE_TO_STRING
...
async def test_status_fan_speed_list(hass, mqtt_mock_entry):
    """Test status updates from the vacuum."""
    config = deepcopy(DEFAULT_CONFIG)
    config[mqttvacuum.CONF_SUPPORTED_FEATURES] = services_to_strings(
        ALL_SERVICES, SERVICE_TO_STRING
...
async def test_status_no_fan_speed_list(hass, mqtt_mock_entry):
    """Test status updates from the vacuum.
...
async def test_status_error(hass, mqtt_mock_entry):
    """Test status updates from the vacuum."""
    config = deepcopy(DEFAULT_CONFIG)
    config[mqttvacuum.CONF_SUPPORTED_FEATURES] = services_to_strings(
        ALL_SERVICES, SERVICE_TO_STRING
...
async def test_battery_template(hass, mqtt_mock_entry):
...
async def test_status_invalid_json(hass, mqtt_mock_entry):
...
async def test_missing_battery_template(hass, mqtt_mock_entry):
...
async def test_missing_charging_template(hass, mqtt_mock_entry):
    """Test to make sure missing template is not allowed."""
    config = deepcopy(DEFAULT_CONFIG)
...
async def test_missing_cleaning_template(hass, mqtt_mock_entry):
    """Test to make sure missing template is not allowed."""
    config = deepcopy(DEFAULT_CONFIG)
...
async def test_missing_docked_template(hass, mqtt_mock_entry):
    """Test to make sure missing template is not allowed."""
    config = deepcopy(DEFAULT_CONFIG)
...
async def test_missing_error_template(hass, mqtt_mock_entry):
    """Test to make sure missing template is not allowed."""
    config = deepcopy(DEFAULT_CONFIG)
...
async def test_missing_fan_speed_template(hass, mqtt_mock_entry):
    """Test to make sure missing template is not allowed."""
    config = deepcopy(DEFAULT_CONFIG)
...
async def test_unique_id(hass, mqtt_mock_entry):
    ...
    await help_test_unique_id(hass, mqtt_mock_entry, vacuum.DOMAIN, config)


async def test_discovery_removal_vacuum(hass, mqtt_mock_entry, caplog):
...
async def test_discovery_update_vacuum(hass, mqtt_mock_entry, caplog):
...
async def test_discovery_update_unchanged_vacuum(hass, mqtt_mock_entry, caplog):
    """Test update of discovered vacuum."""
    ...
    with patch(
        ...
        await help_test_discovery_update_unchanged(
            hass, mqtt_mock_entry, caplog, vacuum.DOMAIN, data1, discovery_update
        )
...
async def test_entity_id_update_subscriptions(hass, mqtt_mock_entry):
    ...
    await help_test_entity_id_update_subscriptions(
        hass, mqtt_mock_entry, vacuum.DOMAIN, config, ["test-topic", "avty-topic"]
    )
...
async def test_entity_debug_info_message(hass, mqtt_mock_entry):
    ...
    await help_test_entity_debug_info_message(
        hass, mqtt_mock_entry, vacuum.DOMAIN, config, vacuum.SERVICE_TURN_ON
    )
